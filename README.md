Resolve README merge conflict and update project documentation
...
1. The user enters a news statement or article in the web form.
2. The application cleans the text using regex, tokenization, and stopword removal.
3. The TF-IDF vectorizer transforms the cleaned text into numerical features.
4. The Logistic Regression model predicts whether the content is rumor or true news.
5. The Google Fact Check Tools API is queried with a short claim to find related fact-check results.
6. The final result is shown with model prediction, confidence score, and API-based fact-check output.
...
```bash
git clone https://github.com/rajamanrazz78-web/Rumour-Detection-System.git
